...
**30초 답변** (150자):
> "워크온의 복제가 아니라 **사용자로서 느낀 두 가지 결핍**을 해결한 실험입니다. (1) 워크온 본 앱은 Health Connect 미통합이라 Samsung Health 데이터를 잃었고, (2) 재부팅·타임존에서 걸음수가 꼬여요. `StepSourceResolver`로 HC 우선 fallback 구조 + `BOOT_COUNT` 기반 baseline 보정으로 두 문제를 풀었습니다."

**60초 답변** (어필 전환 + 디테일):
> "30초 버전 + 'StepSourceResolver는 사용자가 어느 헬스앱을 메인으로 쓰든 데이터 단절 없이 시작하게 해줍니다. 만약 입사하면 워크온이 Health Connect로 이동할 때 이 모듈을 그대로 활용 가능하다고 봤어요. 또한 ADR-001에 의사결정 트레이드오프를 기록해서, 이 결정의 컨텍스트가 사라지지 않도록 했습니다.'"

**유도 질문 (자연스럽게 끌어내기)**:
> "혹시 워크온은 Health Connect 통합 로드맵 있으신가요?"
